...
export const EditorToolbar: React.FC<{
  ...
  return (
    <div className={`absolute z-[999] flex items-center gap-3 ${group && !isTouchActive ? "opacity-0" : "opacity-100"} ${isTouchActive ? "scale-100" : "scale-90"} ${hoverClasses} transition-all duration-300 bg-card/95 backdrop-blur-md border border-border/50 ${isSmall ? "p-1 rounded-lg" : "p-1.5 rounded-xl"} shadow-2xl origin-top-right ${className || "top-2 right-2"}`}>
      ...
      {onToggleProfile && (
        <button onClick={(e) => { e.stopPropagation(); onToggleProfile(); }} className={`${btnPadding} rounded-lg transition-colors ${!profileHidden ? "hover:bg-indigo-500/10 text-indigo-500" : "bg-indigo-500 text-white"}`} title={!profileHidden ? "Hide Profiles (All Cards)" : "Show Profiles (All Cards)"}>
          {!profileHidden ? (
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /><line x1="2" y1="2" x2="22" y2="22" /></svg>
          )}
        </button>
      )}

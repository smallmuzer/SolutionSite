...
const TechnologiesSection = () => {
  ...
  return (
    <section id="technologies" className="section-padding section-alt relative overflow-hidden">
      ...
      <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-border/50 to-transparent"></div>
      <div className="absolute -top-40 -right-40 w-96 h-96 bg-primary/5 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute top-1/2 -left-20 w-72 h-72 bg-secondary/5 rounded-full blur-3xl pointer-events-none"></div>
            ...
            {techs.map((tech, i) => {
              ...
              return (
                <AnimatedSection key={tech.id} delay={i * 0.04}>
                  ...
                  >
                    <div className="absolute -inset-0.5 rounded-xl blur opacity-0 group-hover:opacity-40 transition duration-500" style={{ backgroundColor: nameColor }}></div>
                    <div className="relative h-full glass-card flex flex-col p-4 gap-2.5 group-hover:-translate-y-1 shadow-sm group-hover:shadow-md border border-border/40 hover:border-transparent transition-all duration-300 rounded-xl bg-card/60 backdrop-blur-md overflow-hidden">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-3">
                          <div
                            className="shrink-0 w-10 h-10 rounded-lg flex items-center justify-center p-1.5 shadow-sm transform group-hover:scale-110 transition-transform duration-300 ease-out"
                            style={{ background: `linear-gradient(135deg, ${nameColor}15, ${nameColor}05)`, border: `1px solid ${nameColor}25` }}
                          >
                            {logoSrc ? (
                              <LogoImg src={logoSrc} name={tech.name} size={22} />
                            ) : (
                              <CatIcon size={18} className="text-secondary drop-shadow" />
                            )}
                          </div>
                        ...
                        <span
                          className="shrink-0 text-[0.6rem] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border whitespace-nowrap mt-1"
              ...
              return (
                <AnimatedSection key={tech.id} delay={i * 0.03}>
                  <div
                    ...
                    onClick={scrollToContact}
                  >
                    <div className="absolute -inset-[1px] rounded-xl blur-sm opacity-0 group-hover:opacity-30 transition duration-500" style={{ backgroundColor: nameColor }}></div>
                    <div className="relative glass-card flex flex-col xs:flex-row xs:items-center gap-4 xs:gap-5 px-5 py-4 border border-border/40 hover:border-transparent transition-all duration-300 rounded-xl bg-card/60 backdrop-blur-sm shadow-sm group-hover:shadow-md">
                      <div
                        className="shrink-0 w-12 h-12 rounded-xl flex items-center justify-center p-2.5 shadow-inner transform group-hover:rotate-3 transition-transform duration-300"
                        style={{ background: `linear-gradient(to bottom right, ${nameColor}20, ${nameColor}05)`, border: `1px solid ${nameColor}25` }}
                      >
                        {logoSrc ? (
                          <LogoImg src={logoSrc} name={tech.name} size={28} />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                        <div className="sm:w-1/3 flex flex-col gap-1.5 items-start">
                          ...
                          <span
                            className="text-[0.65rem] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md inline-block border whitespace-nowrap"

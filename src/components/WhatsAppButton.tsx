...
const WhatsAppButton = () => {
  ...
  useEffect(() => {
    const onKoyaStatus = (e: Event) => {
      setKoyaActive((e as CustomEvent).detail === "active");
    };
    window.addEventListener("ss:koyaBotStatus", onKoyaStatus);
    return () => window.removeEventListener("ss:koyaBotStatus", onKoyaStatus);
  }, []);
  ...
  return (
    <>
      ...
      <div 
        id="tour-float-btns" 
        className={`fixed z-50 flex gap-2 ${koyaActive ? "flex-col" : "flex-row items-center"}`}
        style={{
          bottom: koyaActive ? `calc(20px + ${btnSize}px + 12px)` : "1rem",
      ...
      >
        ...
        {!koyaActive && (
          <button
            ...
            style={{ width: `${btnSize}px`, height: `${btnSize}px`, borderRadius: "50%", background: "hsl(217 91% 60%)", color: "#fff", border: "none", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(0,0,0,0.2)", cursor: "pointer", transition: "transform 0.2s ease" }}

...
- ✅ 3. Full-stack ready: Frontend 4173 + Backend 3001
...
[API]   SQLite API: http://localhost:3001  
[PREVIEW] Vite Preview: http://localhost:4173
```

**Verify APIs respond** (no 502s):
```
curl "http://localhost:3001/api/db/site_content?section_key=settings&_single=1"
curl "http://localhost:3001/api/db/seo_settings?page_key=home&_single=1"
curl "http://localhost:3001/api/db/batch?tables=client_logos,services&client_logos_filter={\"is_visible\":true}"
```
...
- [ ] 8. IIS Config: Static files + `/api → localhost:3001`
